...
namespace Vampire
{
    public class JobGiver_VampireBeast : ThinkNode_JobGiver
    {
        ...
        protected override Job TryGiveJob(Pawn pawn)
        {
            ...
            if (pawn2 != null && pawn.CanReach(pawn2, PathEndMode.Touch, Danger.Deadly, false, TraverseMode.ByPawn))
            {
                if (pawn2.InAggroMentalState)
                    ...
                    return this.FeedJob(pawn, pawn2);
            }
        ...
        private Job FeedJob(Pawn pawn, Thing target)
        {
            return new Job(VampDefOf.ROMV_Feed, target)
            {
        ...
        {
            return (Pawn)AttackTargetFinder.BestAttackTarget(pawn, TargetScanFlags.NeedReachable, (Thing x) => x is Pawn p && !p.Dead && p?.BloodNeed().CurBloodPoints > 0 && !p.IsVampire() && (p.PositionHeld.Roofed(p.Map) || !VampireUtility.IsDaylight(p)), 0f, 9999f, default(IntVec3), 3.40282347E+38f, true);
        }
